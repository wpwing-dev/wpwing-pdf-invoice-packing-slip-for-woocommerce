=== WPWing PDF Invoice Packing Slip for WooCommerce ===
Contributors: wpwing, voboghure
Donate link: https://wpwing.com/
Tags: PDF, Invoice, Packing Slip, Proforma Invoice, Delivery Notes, Shipping Labels, WooCommerce, WPWing
Requires at least: 4.7
Tested up to: 5.4
Stable tag: 1.0.0
Requires PHP: 7.0
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html


== Description ==

Generate PDF invoices and packing slips for WooCommerce orders.


== Frequently Asked Questions ==

= Can I contribute to the code? =

You’re more than welcome! This plugin is hosted on github, where you can post issues or make pull requests.


== Screenshots ==

1. Place holders for screenshot. (This screen shot description corresponds to screenshot-1.(png|jpg|jpeg|gif). Screenshots are stored in the /assets directory.)

== Changelog ==

= 1.0.0 =
* Initial release


== Upgrade Notice ==

= 1.0.0 =
* Initial release