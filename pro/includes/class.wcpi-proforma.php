...
if ( ! class_exists( 'WCPI_Proforma' ) ) {
	 ...
	 */
	class WCPI_Proforma extends WCPI_ProDocument {
		...
		private function init_document() {
			...
			$this->exists   = $this->order->get_meta( '_wpwing_wcpi_proforma' );
			if ( $this->exists ) {
				$this->save_path = $this->order->get_meta( '_wpwing_wcpi_proforma_path' );
			}
		}

		public function reset() {
			$this->order->delete_meta_data( '_wpwing_wcpi_proforma' );
			$this->order->delete_meta_data( '_wpwing_wcpi_proforma_path' );
		...
		public function save() {
			...
			$this->order->update_meta_data( '_wpwing_wcpi_proforma', $this->exists );
			$this->order->update_meta_data( '_wpwing_wcpi_proforma_path', $this->save_path );
			$this->order->apply_changes();
			$this->order->save_meta_data();
		...
		public function init_template_generation_actions() {
			add_action( 'wpwing_wcpi_proforma_template_company_data', array( $this, 'show_proforma_template_company_data' ) );
			add_action( 'wpwing_wcpi_proforma_template_company_logo', array( $this, 'show_proforma_template_company_logo' ) );
			add_action( 'wpwing_wcpi_proforma_template_customer_data', array( $this, 'show_proforma_template_customer_data' ) );
			add_action( 'wpwing_wcpi_proforma_template_order_data', array( $this, 'show_proforma_template_order_data' ) );
			add_action( 'wpwing_wcpi_proforma_template_product_list', array( $this, 'show_proforma_template_product_list' ) );
			add_action( 'wpwing_wcpi_proforma_template_footer', array( $this, 'show_proforma_template_footer' ) );
		}
		...
		public function show_proforma_template_order_data() {
			...
			<table>
				<tr class="invoice-number">
					...
					<td class="right"><?php echo esc_html( 'PRO-' . $wpwing_wcpi_document->order->get_order_number() ); ?></td>
				</tr>
