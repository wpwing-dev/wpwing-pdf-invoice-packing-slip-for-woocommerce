# Claude Code Instructions

## Code Comments

- Keep comments **minimal** — only add them when the intent is not obvious from the code itself.
- Prefer **single-line comments** (`//`, `#`, etc.) over block or multi-line comments.
- Never comment on what the code does if the code is already self-explanatory.
- Avoid redundant comments like `// increment i` above `i++`.
- If a comment is needed, make it concise — one short line is the target.

## Punctuation

- Use a plain hyphen (`-`) as a separator in prose and changelogs. Never use an en-dash (`–`) or em-dash (`—`).
